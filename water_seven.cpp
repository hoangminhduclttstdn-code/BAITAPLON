...
int damageEvaluation(int shipHP, int repairCost){
    ...
    return clamp(repairCost, 0, 3000);
}
...
int conflictSimulation(
    ...
    int shipHP, int repairCost){
        ...
        double result = (double)skillLuffy - skillUsopp + (repairCost / 100.0) + ((500.0 - shipHP) / 50.0);
        int conflictIndex = (int)ceil(result);
...
void decodeCP9Message(char character[FIXED_CHARACTER][MAX_NAME], 
   ...
   int n = strlen(cipherText);
    ...
    int sumASCII = 0;
    for (int i = 0; i < posHashtag; i++) {
        sumASCII += (int)((unsigned char)cipherText[i]);
    }
    

    int X = cipherText[n - 2] - '0';
    int Y = cipherText[n - 1] - '0';
    int XY = X * 10 + Y;
    ...
    for (int i = 0; i < posHashtag; i += B) {
        int start = i;
        int end = i + B - 1;
        if (end >= posHashtag) end = posHashtag - 1; 
    ...
    for (int i = 0; i < posHashtag; i++) {
        if (message[i] >= 'A' && message[i] <= 'Z') {
            ...
            message[i] = (message[i] - '0' - shiftDigit + 10) % 10 + '0';
        }
